...
## ポートフォリオに反映すべきコンテンツ（アップロード資料より）

### 研究内容（About/Profile）
- テーマ：深層学習を用いたロボット動作生成
- 目的：人とロボットが共生する社会の実現
- 内容：双腕の部分拘束協調動作に着目した研究
- 技術：階層型深層予測学習モデル（LSTM層 + Union-LSTM層）
- 成果：皿洗い動作実験において優れた汚れ除去率を達成
- 社会的意義：高齢化社会における介護・家事支援

### 開発経験（Works）

#### 1. Azure AIサービスを活用したRAGシステム
- 開発目的：長期インターン
- 開発期間：2024年12月〜2025年1月（約2ヶ月間）
- 技術スタック：
  - フロントエンド：React, MUI, TypeScript, Azure Static Web Apps
  - バックエンド：Python, Azure Functions, FastAPI, LangChain
  - AI・ML：Azure OpenAI Service, Azure AI Search, Azure Document Intelligence
  - インフラ：Microsoft Azure, SharePoint Online, Azure Cosmos DB
- 概要：企業向けドキュメント検索・質問応答システム（RAG技術活用）
- 特徴：
  - Azure Document Intelligenceによる高精度OCR
  - セマンティックチャンキング法とベクトル検索
  - SharePoint連携による自動ドキュメント同期
  - フォルダ階層でのフィルタリング機能
- 役割：4人チームのプロジェクトリーダー、バックエンド開発担当
- 成果：エラー対応の標準化により開発速度を約2倍に向上、2ヶ月でのリリース実現
- URL：https://github.com/saoki0913/azure_rag

#### 2. スケジュール管理システム
- 開発目的：長期インターン
- 開発期間：2025年1月〜2025年3月（約3ヶ月間）
- 技術スタック：
  - フロントエンド：Next.js, React, TypeScript, Tailwind CSS
  - バックエンド：Python, Azure Functions, FastAPI, Pydantic
  - API・認証：Microsoft Graph API, MSAL
  - データベース：Azure Cosmos DB
- 概要：面接スケジュール調整を自動化するWebアプリケーション
- 特徴：
  - 複数の面接官のOutlookカレンダーから空き時間を自動検出
  - 自動でTeamsミーティングを作成
  - Microsoft Graph APIとの連携
- URL：https://github.com/saoki0913/schedule_management

#### 3. ポートフォリオサイト
- 開発目的：趣味/技術習得
- 開発期間：2025年2月〜2025年3月（約1ヶ月間）
- 技術スタック：
  - フロントエンド：Next.js, React, TypeScript, Tailwind CSS
  - バックエンド：FastAPI, Pydantic, Uvicorn
  - データベース：Supabase (PostgreSQL)
  - インフラ：Vercel (フロントエンド), Render (バックエンド)
- 概要：自身の活動を記載するポートフォリオサイト
- 特徴：動的コンテンツ管理、レスポンシブデザイン
- URL：https://github.com/saoki0913/portfolio

### 長期インターン経験（Hero/Timeline）

#### 1. 株式会社インテリジェントフォース
- 期間：2024年10月〜2025年6月（9ヶ月）
- 役割：開発チームのリーダー
- 業務内容：Microsoft Azureを活用した社内向けRAGチャットシステムの開発
- 技術スタック：Python (FastAPI), JavaScript (React), LangChain, Azure OpenAI Service, Azure AI Search, Azure Functions, Azure Static web apps
- 主な成果：
  - Azure Document IntelligenceによるOCRとセマンティックチャンキングの導入
  - Web開発未経験者4名のチームでリーダーを務め、開発速度を約2倍に向上
  - 2ヶ月でのリリースを実現
  - SharePoint Onlineとの自動同期機能を実装

#### 2. 株式会社EQUES
- 期間：2025年2月〜現在（約11ヶ月）
- 役割：フルスタックエンジニア
- 業務内容：製薬業界向け業務文書自動生成SaaSの開発
- 技術スタック：Python (FastAPI), TypeScript (Next.js), AWS, PostgreSQL, Docker, MUI
- 担当業務：
  - バックエンド：OpenAI/Azure OpenAIを活用したAI文書生成機能、AWS Cognito認証システム、クリーンアーキテクチャに基づいた設計
  - フロントエンド：Next.js + MUIによるUI/UX実装、型安全な非同期通信処理
  - インフラ：AWS連携（Cognito, S3, SES, Kinesis Firehose）、Docker/Makefile活用
  - 品質管理：Mypy, Flake8による静的解析、Vitest/Playwrightによるテスト
...
#### クラウド・インフラ
- Microsoft Azure（Azure Functions, Azure Static Web Apps, Azure OpenAI Service, Azure AI Search, Azure Document Intelligence）
- AWS（Cognito, S3, SES, Kinesis Firehose）
- Vercel
- Render
- Docker
...
### 2.2 コンテンツデータの投入
上記「ポートフォリオに反映すべきコンテンツ」セクションの内容をSupabaseに投入：

#### worksテーブル
- Azure AIサービスを活用したRAGシステム
- スケジュール管理システム
- ポートフォリオサイト

#### heroテーブル（Timeline/経歴）
- 株式会社インテリジェントフォース（2024年10月〜2025年6月）
- 株式会社EQUES（2025年2月〜現在）

#### skillsテーブル
- プログラミング言語（Python, TypeScript/JavaScript, SQL）
- フロントエンド技術
- バックエンド技術
- データベース
- クラウド・インフラ
